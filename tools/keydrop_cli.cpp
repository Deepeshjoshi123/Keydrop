...
int main(int argc, char** argv) {
    ...
    if (command == "encode" && argc == 5) { std::vector<ConfiguredSchema>s; if(!load(argv[2],s))return 1;auto schema=find_schema(s,argv[3]);if(!schema){std::cerr<<"unknown schema\n";return 1;}JsonObject j;if(!load_payload(argv[4],j))return 1;SchemaRuntime runtime;register_configured_schema(runtime,*schema);Buffer b;auto sent=runtime.send_json(schema->schema.schema_name,j,b);if(!sent.ok()){std::cerr<<sent.message<<"\n";return 1;}std::cout<<hex(b)<<"\n";return 0; }
    if (command == "decode" && argc == 4) { std::vector<ConfiguredSchema>s; if(!load(argv[2],s))return 1;Buffer b;if(!unhex(argv[3],b)){std::cerr<<"invalid packet hex\n";return 1;}SchemaRuntime runtime;for(const auto&x:s)register_configured_schema(runtime,x);std::string name;JsonObject j;auto r=runtime.receive_json(b,name,j);if(!r.ok()){std::cerr<<r.message<<"\n";return 1;}std::cout<<format_json_object(j)<<"\n";return 0; }
    usage(); return 2;
}
